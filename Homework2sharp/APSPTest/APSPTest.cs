...
using System;
using MyBoolean = MatrixProcessor.MyBoolean;
...
namespace APSPTest
{
    ...
    public class APSPTest
    {
        ...
        public void TransitiveClousureTest()
        {
            ...
            Matrix<MyBoolean> origin = new Matrix<MyBoolean>(4, 4);
            Matrix<MyBoolean> expected = new Matrix<MyBoolean>(4, 4);

            for (int i = 0; i < array0.GetLength(0); i++)
                for (int j = 0; j < array0.GetLength(1); j++)
                {
                    origin[i, j] = new MyBoolean(array0[i, j]);
                    expected[i, j] = new MyBoolean(array1[i, j]);
                }

            Matrix<MyBoolean> result = TransitiveClosureProcessor.Process(origin);
